...
module Types
  class MutationType < Types::BaseObject
    ...
    field :create_channel_lookup, Types::ChannelLookupType, null: false do
      ...
      argument :channel_id, String, required: false, description: "YouTube channel ID"
      argument :channel_name, String, required: false, description: "Name of the YouTube channel"
    end
    def create_channel_lookup(channel_id: nil, channel_name: nil)
      fetcher = YoutubeFetcher.new
      begin
        # Fetch channel data from YouTube API
        channel_data = fetcher.fetch_channel(channel_id, channel_name)
      rescue StandardError => e
        Rails.logger.error("Error fetching channel data: #{e.message}")
      end
      return unprocessable_entity(channel_data.errors) if channel_data.nil?
